...
class DataIngestionEngine:
    ...
    def _switch_to_next_key(self) -> bool:
        ...
        if self.current_key_index < len(self.api_keys) - 1:
            ...
            self.api_key = self.api_keys[self.current_key_index]
            print(
                f"⚠️ Rate limit hit. Switching to backup API key #{self.current_key_index + 1}"
            )
            return True
        else:
            print(
                f"❌ All {len(self.api_keys)} API key(s) have hit rate limits")
            return False
    ...
    def fetch_weekly_data(self,
        ...
        try:
            ...
            if 'Note' in data or 'Information' in data:
                ...
                if self._switch_to_next_key() and retry_count < len(
                        self.api_keys):
    ...
    def fetch_fundamental_data(self,
        ...
        try:
            ...
            if 'Note' in data or 'Information' in data:
                rate_limit_msg = data.get('Note') or data.get('Information')
                print(
                    f"API Rate Limit (fundamentals) for {symbol}: {rate_limit_msg}"
                )
                ...
                if self._switch_to_next_key() and retry_count < len(
                        self.api_keys):
                    print(
                        f"  → Retrying {symbol} fundamentals with backup key..."
                    )
                    time.sleep(1)  # Brief pause before retry
